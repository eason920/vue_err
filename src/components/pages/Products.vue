...
export default {
	...
	methods: {
		...
		updateProduct(){
			const vm = this;
			const api = `${process.env.API_PATH}api/${process.env.ACT_PATH}/admin/product`;
			const obj = {
				"data": {
					"title": "[賣]動物園造型衣服3",
					"category": "衣服2",
			...
			console.log(api, vm.tempProduct);
			console.log( typeof(vm.tempProduct) );
			// vm.$http.post(api, obj).then((res)=>{
			vm.$http.post(api, {data: vm.tempProduct}).then((res)=>{
				console.log(res);
				// vm.getProducts;
			});
		}
